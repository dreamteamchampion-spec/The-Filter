  ...
  try {
    ...
    const upstream = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        // Raised from 2000, then from 8000. This is a CEILING, not a target
        // — output tokens are billed only as actually used, so a higher cap
        // costs nothing extra on short replies. It only stops long ones
        // being truncated. 2000 was fine until C-stage responses grew: the
        // Still Open field, the fuller prep briefings, and long post-date
        // debriefs push a full card (reasoning + gate + frame + move +
        // angle + bids + flags + three tiered replies) past 2000 tokens.
        // 8000 broke too, but for a different reason: claude-sonnet-5 has
        // adaptive thinking on by default, and max_tokens is a hard cap on
        // TOTAL output — thinking plus visible text combined. A long,
        // tangled input (multiple embedded questions, reported evidence to
        // log) makes the model think more before it writes, which alone
        // can eat most of an 8000 budget before the card even starts. Fixed
        // properly below by turning thinking off, since this app's stream
        // parser only reads text_delta events and drops thinking silently
        // anyway — it was never going to be shown. 32000 leaves generous
        // headroom for the biggest realistic card, still costs nothing on
        // an ordinary short reply, and stays far under Sonnet 5's 128k
        // ceiling. When it truncates it fails mid-JSON, so the card breaks
        // rather than simply ending early.
        max_tokens: 32000,
        // claude-sonnet-5 runs adaptive thinking by default. This app was
        // built for a model where max_tokens applied only to the visible
        // response, and the client never learned to read thinking_delta
        // events — so on Sonnet 5, thinking silently ate into the same
        // budget as the card, with nothing to show for it. Disabling it
        // restores the original behaviour: max_tokens governs the visible
        // reply only, deterministically.
